...
public class LordCommanderOfTheNorthAbility implements LeaderAbility {
    ...
    private void removeMaxPoweredCardInARow(Player player, Row row) {
        if (!row.isEmpty() && row.getStrength() >= 10) {
            ...
            ((GameMenuControllerForOnlineGame) Controller.GAME_MENU_CONTROLLER_FOR_ONLINE_GAME.getController()).moveCardAndDontDoAbilityForCurrentPlayer(maxPoweredCard.getIdInGame(), RowsInGame.opponentSiegeSpecialPlace.toString(), RowsInGame.opponentDiscardPlace.toString());
        }
    }
}
